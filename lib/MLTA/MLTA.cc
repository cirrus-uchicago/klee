...
#include "klee/MLTA/Common.h"
#include "klee/MLTA/MLTA.h"
...
bool MLTA::fuzzyTypeMatch(Type *Ty1, Type *Ty2, 
	...
	if (Ty1->isStructTy() && Ty2->isStructTy() &&
			(get_real_structure_name(Ty1->getStructName().str()) == get_real_structure_name(Ty2->getStructName().str())))
		return true;
...
void MLTA::unrollLoops(Function *F) {
	...
	for (LoopInfo::iterator i = LI->begin(), e = LI->end(); i!=e; ++i) {
		...
		while (!LPL.empty()) {
			LP = LPL.front();
			LPL.pop_front();
			std::vector<Loop *> SubLPs = LP->getSubLoops();
...
bool MLTA::typeConfineInInitializer(GlobalVariable *GV) {
	...
	std::list<std::pair<Type *, int>>NestedInit;
	std::map<Value *, std::pair<Value *, int>>ContainersMap;
	std::set<Value *>FuncOperands;
	std::list<User *>LU;
	...
	while (!LU.empty()) {
		...
		for (auto oi = U->op_begin(), oe = U->op_end(); 
			...
			if (FoundF && !FoundF->isIntrinsic()) {
				...
				Value *CV = O;
				std::set<Value *>Visited; // to avoid loop
...
void MLTA::collectAliasStructPtr(Function *F) {
	...
	std::map<Value *, Value *> &AliasMap = AliasStructPtrMap[F];
...
void MLTA::saveCalleesInfo(CallInst *CI, FuncSet &FS,
	...
	std::string CallerFN = SP->getFilename().str();
	...
	for (auto F : FS) {
		...
		std::string CalleeFN = CalleeSP->getFilename().str();
#ifdef EVAL_FN_FIRFOX
...
void MLTA::printTypeChain(std::list<typeidx_t> &Chain) {
	if (Chain.empty())
		return;

	for (std::list<typeidx_t>::iterator it = Chain.begin(); 
			it != Chain.end(); ++it) {
		typeidx_t TI = *it;
...
// propagate further
bool MLTA::getBaseTypeChain(std::list<typeidx_t> &Chain, Value *V,
		bool &Complete) {
...
Type *MLTA::getBaseType(Value *V, std::set<Value *> &Visited) {
...
Type *MLTA::_getPhiBaseType(PHINode *PN, std::set<Value *> &Visited) {
...
bool MLTA::getGEPLayerTypes(GEPOperator *GEP, std::list<typeidx_t> &TyList) {

	Value *PO = GEP->getPointerOperand();
	Type *ETy = GEP->getSourceElementType();

	std::vector<int> Indices; 
	std::list<typeidx_t> TmpTyList;
...
bool MLTA::nextLayerBaseTypeWL(Value *V, std::list<typeidx_t> &TyList,
		Value * &NextV) {
...
// memory loads or GEP
bool MLTA::nextLayerBaseType(Value *V, std::list<typeidx_t> &TyList, 
...
bool MLTA::getDependentTypes(Type *Ty, int Idx, 
		std::set<hashidx_t> &PropSet) {

	std::list<hashidx_t>LT;
	LT.push_back(hashidx_c(typeHash(Ty), Idx));
	std::set<hashidx_t>Visited;
